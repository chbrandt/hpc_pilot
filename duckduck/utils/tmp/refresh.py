    ...
    while True:
        ...
        if iam_grant_type == "client_credentials":
            try:
                ...
                r = requests.post(iam_server, data=request_data,
                ...
                logger.debug("iam_client_id %s iam_client_secret %s response %s",
                             iam_client_id, iam_client_secret, response)
                ...
                logger.info(
                    f"Refresh token written in {output_file+'-refresh'}")

            except Exception as e:
                logger.warning(
                    "ERROR oidc get token: {}".format(e), exc_info=True)
